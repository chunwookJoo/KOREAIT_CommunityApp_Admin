 ...
 *
 * 호출 (POST), https://app.koreait.kr/article/reply/ismine/
 * 		Key					Value					설명			비고
 * 		==========================================================================
 *		reply_id			8						댓글 고유 번호	필수
 ...
 * 		RESULT				100						일치
 * 							400						댓글 없음
 * 							410						학번 틀림		게시 시 학번 필요
 * 							500						내부 오류
 *
 */
class _ApiReplyIsMine extends Controller
{
	public function __invoke(Request $request)
	{
		try {
			$db_result = DB::select(
				'CALL koreaitedu.reply_is_mine(?,?);',
				array(
					$request->reply_id,
					$request->user_id,
				)
